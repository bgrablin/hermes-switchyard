...
- Hermes 0.19 status and live collector: tolerate missing `agent.skill_utils.parse_config_string_list` so `hermes switchyard status` resolves platform selection and reports real `callable` values; tolerate missing `hydrate_profile_secret_sources` and fall back to a process `OPENROUTER_API_KEY` when building the collector secret scope.
