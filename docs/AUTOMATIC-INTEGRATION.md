...
Every finished turn also appends one record to `receipt-history.jsonl`, the bounded per-turn history next to the latest-receipt file in the profile plugin-data directory. Each record holds the canonical receipt plus `session_id`, `turn_id`, `platform`, and a UTC `recorded_at` timestamp. Task text, conversation history, candidate descriptions, and provider text are never stored. The file retains at most 500 valid records and 1 MiB. Ordinary turns append one line under a bounded-wait lock without an `fsync` on the hook's critical path; a duplicate, corrupt tail, or limit crossing is compacted with an atomic replacement and private permissions. History is best-effort diagnostic data, not a durable audit log.
...
| `automatic_skill_platforms` | `[]` | Empty uses the interactive policy, which skips API-server, batch, cron, webhook, and Kanban worker turns. `[all]` opts in every platform. A list such as `[cli, telegram]` routes only those platforms; a Kanban worker also requires `kanban` in the list. Invalid non-empty overrides refuse automatic routing instead of silently broadening the policy. |
