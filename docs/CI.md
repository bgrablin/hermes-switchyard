...
`.github/workflows/offline-ci.yml` runs the plugin's Python test and hygiene surface on Ubuntu and Windows with Python 3.11, 3.12, and 3.13. The matrix uses the supported Hermes range `>=3.11,<3.14`; Python 3.14 is not a supported Hermes runtime for this gate.

Each matrix job:

1. Checks out Hermes Agent at `8503ee4459316ce092b5d69b7d396c27aa03d0be`.
...
Local equivalent from a Hermes Agent checkout at the pinned SHA:

```text
HERMES_VENV="$(mktemp -d)/hermes-switchyard-ci-venv"
uv venv "$HERMES_VENV" --python 3.11
uv pip install --python "$HERMES_VENV/bin/python" -e ".[all,dev]"
HERMES_HOME="$(mktemp -d)" "$HERMES_VENV/bin/hermes" plugins validate --json .
HERMES_HOME="$(mktemp -d)" "$HERMES_VENV/bin/hermes" plugins doctor --ci .
REPORT="$(mktemp)"
HERMES_HOME="$(mktemp -d)" "$HERMES_VENV/bin/python" scripts/ci/check_native_hermes.py \
  --plugin-root . \
  --upstream-root /path/to/hermes-agent \
...
HERMES_HOME="$(mktemp -d)" \
  /path/to/hermes-python scripts/ci/live_jev_contract.py \
  --plugin-root . \
  --upstream-root /path/to/hermes-agent \
  --source-sha "$(git rev-parse --verify HEAD)" \
  --upstream-sha 8503ee4459316ce092b5d69b7d396c27aa03d0be \
