...
| Release archive | Exact Git-blob payload allowlist, deterministic bytes, strict source SHA, embedded checksums, fresh extraction, optional external source reference | `SOURCE_SHA="$(git rev-parse --verify HEAD)"; python scripts/build_release.py --source-sha "$SOURCE_SHA"` and `tests/test_release.py` | The ZIP excludes tests, evaluation data, CI files, caches, logs, and local evidence. `--verify` with `--source-root` and `--expected-source-sha` distinguishes source verification from internal integrity checks. |
| Linux and Windows Python matrix | Python 3.11, 3.12, and 3.13 on Linux and Windows | `.github/workflows/offline-ci.yml` | Six offline jobs are defined; a fresh CI run is required for a release claim. |
